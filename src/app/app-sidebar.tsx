...
import { defaultPlanCode, getPlanByCode } from "@/lib/plans";
...
export function AppSidebar() {
  ...
  const currentPlan = getPlanByCode(defaultPlanCode, t, locale);
  ...
  function isActive(item: NavItem) {
    ...
    if (item.labelKey === "projects" && pathname.startsWith("/projects/"))
      return true;
    return false;
  }
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`transition-colors ${active ? "text-brand-400" : "text-surface-500 group-hover:text-surface-300"}`}
              >
                {item.icon}
              </span>
