...
export default function NewProject() {
  ...
  // Fetch voices when voice is enabled
  useEffect(() => {
    if (!voiceEnabled) return;
    if (voices.length > 0) return; // already fetched

    let cancelled = false;
    setLoadingVoices(true);

    fetch(getApiUrl("/api/elevenlabs/voices"))
      .then((res) => res.json())
      .then((data) => {
        if (cancelled) return;
      ...
      .catch(() => {
        if (!cancelled) setVoicesConfigured(false);
      })
      .finally(() => {
  ...
  }, [voiceEnabled, voices.length]);

  // Stop preview audio on unmount or voice toggle off
  useEffect(() => {
    if (!voiceEnabled && previewAudioRef.current) {
  ...
  // Reset voice selection when language changes if voice is incompatible
  useEffect(() => {
    if (!voiceId || !voiceEnabled) return;
    const selected = voices.find((v) => v.voiceId === voiceId);
    if (
      selected &&
      selected.languages.length > 0 &&
      !selected.languages.some((l) => language.startsWith(l) || l.startsWith(language))
    ) {
      setVoiceId(null);
    }
  }, [language, voiceId, voiceEnabled, voices]);

  // Filter voices by language
  const filteredVoices = voices.filter((v) => {
    if (v.languages.length === 0) return true; // multilingual or no metadata
    return v.languages.some((l) => language.startsWith(l) || l.startsWith(language));
  });
  const showAllVoicesFallback =
    voiceEnabled && voices.length > 0 && filteredVoices.length === 0;
  const displayedVoices = showAllVoicesFallback ? voices : filteredVoices;

  function handlePlayPreview(vId: string, previewUrl: string) {
  ...
  function startRecordingFromStream(stream: MediaStream) {
    ...
    recorder.onstop = () => {
      ...
      const recordedFile = new File([blob], `screen-recording-${ts}.webm`, {
        type: "video/webm",
      });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(await readErrorMessage(res, t.failedCreate));
      }
      ...
      const uploadRes = await fetch(getApiUrl(`/api/projects/${project.id}/upload`), {
        method: "POST",
        body: formData,
      });

      if (!uploadRes.ok) {
        throw new Error(await readErrorMessage(uploadRes, t.failedUpload));
      }
      ...
      if (
        ...
        await fetch(getApiUrl(`/api/videos/${uploadData.id}`), {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="max-w-xl mx-auto animate-fade-in">
      ...
      <p className="text-sm text-surface-400 mb-8">{t.newProjectDesc}</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-7">
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {(
              [
                { id: "en" as const, label: t.languageEn, flag: "EN" },
                { id: "es" as const, label: t.languageEs, flag: "ES" },
                { id: "pt" as const, label: t.languagePt, flag: "PT" },
              ] as const
            ).map((lang) => (
              <button
                key={lang.id}
                ...
                className={`flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl border text-sm font-medium transition-all ${
                  language === lang.id
                    ? "border-brand-500/50 bg-brand-500/8 ring-1 ring-brand-500/20 text-brand-300"
                    : "border-surface-700 bg-surface-900/50 hover:border-surface-600 hover:bg-surface-800/50 text-surface-300"
                }`}
              >
        ...
        <div>
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                  voiceEnabled ? "bg-brand-500/15" : "bg-surface-800"
                }`}
              >
                <svg
                  width="18"
                  ...
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  className={voiceEnabled ? "text-brand-400" : "text-surface-500"}
                >
              ...
              <div>
                <span
                  className={`text-sm font-semibold block ${
                    voiceEnabled ? "text-brand-300" : "text-surface-300"
                  }`}
                >
                  {t.voiceToggleLabel}
                </span>
            ...
            <button
              ...
              className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${
                voiceEnabled ? "bg-brand-500" : "bg-surface-700"
              }`}
            >
              <span
          ...
          {voiceEnabled && (
            <div className="mt-3 space-y-3">
              {loadingVoices ? (
                <div className="flex items-center justify-center gap-2 py-6 text-surface-400 text-sm">
                  <svg
                    className="animate-spin h-4 w-4"
                    viewBox="0 0 24 24"
                    fill="none"
                ...
                <>
                  {showAllVoicesFallback && (
                    <p className="text-xs text-amber-400/80 px-1">
                      {t.noVoicesForLanguage}
                    </p>
                  )}
                  <label className="block text-sm font-medium text-surface-400 px-1">
                    {t.selectVoice}
                  </label>
                  <div className="grid gap-2 max-h-64 overflow-y-auto pr-1">
                    {displayedVoices.map((voice) => {
                      const isSelected = voiceId === voice.voiceId;
                      return (
                        <div
                          key={voice.voiceId}
                          ...
                          <div className="flex-1 min-w-0">
                            <span
                              className={`text-sm font-medium block ${
                                isSelected ? "text-brand-300" : "text-surface-200"
                              }`}
                            >
                              {voice.name}
                            </span>
