...
const icons = {
  ...
  pause: (
    <svg aria-hidden="true" width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="4" width="4" height="16" /><rect x="14" y="4" width="4" height="16" /></svg>
  ),
  skipBack: (
...
export default function Timeline({
  ...
  // Find the closest neighbor boundaries for a subtitle (simple blocking, no cascade).
  function findNeighborBounds(subId: string): { leftBound: number; rightBound: number } {
    const others = subtitles.filter((s) => s.id !== subId).sort((a, b) => a.startTime - b.startTime);
    const self = subtitles.find((s) => s.id === subId);
    if (!self) return { leftBound: 0, rightBound: videoDuration };

    let leftBound = 0;
    let rightBound = videoDuration;

    for (const s of others) {
      if (s.endTime <= self.startTime + 0.01) {
        leftBound = Math.max(leftBound, s.endTime);
      } else if (s.startTime >= self.endTime - 0.01) {
        rightBound = Math.min(rightBound, s.startTime);
        break;
      }
    }
  ...
  function handlePointerMove(e: React.PointerEvent) {
    ...
    const { leftBound, rightBound } = findNeighborBounds(dragState.targetId);
    ...
    if (sub && (Math.abs(newStart - origStart) > 0.01 || Math.abs(newEnd - origEnd) > 0.01)) {
      ...
      try {
        const res = await fetch(
          getApiUrl(`/api/projects/${projectId}/subtitles/${subId}`),
          {
            method: "PATCH",
  ...
  return (
    <div className="hidden lg:block mt-4 select-none touch-manipulation">
      ...
      <div className="flex items-center gap-1 mb-1.5 px-1 py-1 bg-surface-900/80 border border-surface-800 rounded-lg">
        ...
        {/* Playback */}
        <TBtn onClick={skipBack} title={t.skipBack || "Back 5s"} className="w-8 h-8">{icons.skipBack}</TBtn>
        <TBtn onClick={togglePlay} title={t.playPause || "Play/Pause"} className="w-9 h-9 bg-surface-800 hover:bg-surface-700 rounded-lg">
          {isPlaying ? icons.pause : icons.play}
        </TBtn>
        <TBtn onClick={skipForward} title={t.skipForward || "Forward 5s"} className="w-8 h-8">{icons.skipForward}</TBtn>

        {/* Timecode */}
        <span className="text-xs font-mono text-surface-300 tabular-nums ml-2">
          {formatTimecode(playheadPos)}
        </span>
        <span className="text-[11px] text-surface-600 mx-0.5">/</span>
                  ...
                  return (
                    <div
                      ...
                      onPointerDown={(e) => {
                        ...
                        // No move drag — reorder from subtitle list instead
                      }}
                      role="button"
                      tabIndex={0}
