...
function FaqItem({ question, answer }: { question: string; answer: string }) {
  ...
  return (
    <div className="border-b border-surface-800 last:border-0">
      ...
      >
        <p className="text-sm text-surface-400 leading-relaxed pb-5">
          {answer}
        </p>
      </motion.div>
    </div>
...
export default function LandingPage() {
  ...
  const plans = getPlanCatalog(t, locale);
  ...
  return (
    <div className="-mx-6 -mt-8">
            ...
            <div className="p-6 sm:p-8">
              ...
              <div className="mt-4 flex gap-1.5">
                {[
                  ...
                  {
                    flex: "flex-[3]",
                    text: "Click on File and select New",
                    delay: 1.0,
                  },
                  {
                    flex: "flex-[2]",
                  ...
                  >
                    <span className="text-[10px] text-brand-300 truncate">
                      {seg.text}
                    </span>
                  </motion.div>
                ))}
      ...
      <section
        id="features"
        className="px-6 py-24 sm:py-32 border-t border-surface-800/60"
      >
        <div className="max-w-5xl mx-auto">
          <Reveal className="text-center mb-16">
      ...
      <section
        id="pricing"
        className="px-6 py-24 sm:py-32 border-t border-surface-800/60 overflow-hidden"
      >
        <div className="max-w-6xl mx-auto relative">
          <div className="absolute inset-x-0 top-10 -z-10 h-64 bg-[radial-gradient(circle_at_top,rgba(224,122,95,0.12),transparent_60%)]" />

          <Reveal className="text-center mb-16">
            <SectionLabel text={t.pricingLabel} />
            <h2 className="text-3xl sm:text-4xl font-bold text-surface-100 tracking-tight mt-4 mb-4">
              {t.pricingTitle}
            </h2>
            ...
            {plans.map((plan, index) => (
              <Reveal key={plan.code} delay={index * 0.08}>
                <motion.div
                  whileHover={{ y: -6 }}
                  transition={{ duration: 0.25 }}
                  className={`relative h-full rounded-[28px] border p-7 ${
                    plan.featured
                      ? "border-brand-400/40 bg-gradient-to-b from-brand-500/18 via-surface-900 to-surface-950 shadow-2xl shadow-brand-500/10"
                      : "border-surface-800 bg-surface-900/50"
                  }`}
                >
                  <div className="flex items-start justify-between gap-4 mb-8">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.22em] text-brand-300/90 mb-3">
                        {plan.badge}
                      </p>
                    ...
                    {plan.featured ? (
                      <span className="shrink-0 rounded-full border border-brand-300/30 bg-brand-400/15 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-brand-100">
                        {t.pricingFeatured}
                      </span>
                    ) : null}
                  </div>

                  <p className="text-sm leading-relaxed text-surface-400 min-h-[72px]">
                    {plan.description}
                  </p>

                  <div className="mt-8 flex items-end gap-2">
                    <span className="text-4xl font-black tracking-tight text-surface-100">
                      {plan.priceLabel}
                    </span>
                    <span className="pb-1 text-sm text-surface-500">
                      {t.pricingPerMonth}
                    </span>
                  </div>
                  ...
                  <Link
                    ...
                    className={`mt-8 inline-flex w-full items-center justify-center rounded-xl px-4 py-3 text-sm font-semibold transition-all ${
                      plan.featured
                        ? "bg-brand-500 text-white hover:bg-brand-400 hover:shadow-xl hover:shadow-brand-500/20"
                        : "border border-surface-700 bg-surface-950/60 text-surface-200 hover:border-surface-600 hover:bg-surface-900"
                    }`}
                  >
                    {plan.cta}
                  </Link>

                  <div className="mt-8 space-y-3 border-t border-surface-800/80 pt-6">
                    {plan.features.map((feature) => (
                      <div
                        key={feature}
                        className="flex items-start gap-3 text-sm text-surface-300"
                      >
                        <span className="mt-0.5 inline-flex h-5 w-5 items-center justify-center rounded-full bg-brand-500/12 text-brand-300">
                          <svg
                            width="12"
                            height="12"
                            viewBox="0 0 12 12"
      ...
      >
        <div className="max-w-2xl mx-auto">
          <Reveal className="text-center mb-12">
            ...
            <p className="text-surface-400 text-lg">{t.faqSubtitle}</p>
          </Reveal>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="grid sm:grid-cols-4 gap-10 mb-12">
            ...
            <div>
              ...
              <ul className="space-y-2.5">
                ...
                <li>
                  <span className="text-sm text-surface-600 cursor-default">
                    {t.footerChangelog}
                  </span>
                </li>
              </ul>
          ...
          <div className="border-t border-surface-800/60 pt-6 flex items-center justify-between">
            <p className="text-xs text-surface-600">
              &copy; {new Date().getFullYear()} Showguide. All rights reserved.
            </p>
          </div>
        </div>
      </footer>
